...
package com.golaszewski.hash_mechanic.hashes.thirdparty;
...
public class MD5 {
	...
	private int[] Decode(byte buffer[], int len, int shift) {
		...
		for (i = j = 0; j < len; i++, j += 4) {
			...
			/*
			 * System.out.println("out[" + i + "] = \t" + ((int) buffer[j + 0 +
			 * shift] & 0xff) + "\t|\t" + ((int) buffer[j + 1 + shift] & 0xff) +
			 * "\t|\t" + ((int) buffer[j + 2 + shift] & 0xff) + "\t|\t" + ((int)
			 * buffer[j + 3 + shift] & 0xff));
			 */
		}
	...
	public void Update(MD5State stat, byte buffer[], int offset, int length) {
		 ...
		 * "\t"); System.out.print("Buffer = "); for (i = 0; i < buffer.length;
		 * i++) System.out.print((int) (buffer[i] & 0xff) + " ");
		 * System.out.print("\n");
		 */
